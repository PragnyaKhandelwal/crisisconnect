...
function ruleTriage(text) {
  ...
  return { type, people: Math.max(1, people), urgency, note: text.slice(0, 200), source: 'rules' };
}
...
async function triage(text) {
  ...
  if (key) {
    try {
      ...
      if (TYPES[p.type] && p.people > 0) return { type: p.type, people: p.people | 0, urgency: Math.min(5, Math.max(1, p.urgency | 0 || 3)), note: String(p.note || text).slice(0, 200), source: 'groq' };
